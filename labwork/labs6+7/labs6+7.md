...
Task 1.1 involves updating `dijkstra_5_14` to return the predecessor map (in addition to the optimal distances). This is reasonably straightforward and requires no new background.

Note that the resulting function should conform to the interface of NetorkX's `dijkstra_predecessor_and_distance` i.e. having the same order- and types of output arguments.

### Forwarding tables

Slide `5-17` displays the shortest-path forwarding table for Node `u` of using the predecessor map computed on slide `5-16`. Task 1.2 asks us to automate the process.

Note that we are not required to use an optimal algorithm and may visit each node more than once.

- Individually, determine the types of the input and output argument that would be consistent with `dijkstra_predecessor_and_distance` and slide `5-17`; then please move on with this worksheet.
...
We have considered [shortest path problems](https://en.wikipedia.org/wiki/Shortest_path_problem)
 in Lab 1 and in SOFTENG 250. ([Longest path problems](https://en.wikipedia.org/wiki/Longest_path_problem) arise in [project management](https://en.wikipedia.org/wiki/Critical_path_method) and in [graph drawing](https://en.wikipedia.org/wiki/Graph_drawing), but we won't consider these any further).

**Widest-path** problems also arise naturally in network routing. Please acquaint yourself with this problem through the following resources:
- The introduction of the entry at [wikipedia.org](https://en.wikipedia.org/wiki/Widest_path_problem)
...
- Relate the same elements of both formulae to analogs in `dijkstra_5_14`. We should be able to complete the second column of the following table:
...
[Named parameters](https://en.wikipedia.org/wiki/Named_parameter) are supported [several](https://en.wikipedia.org/wiki/Named_parameter#Use_in_programming_languages) programming languages, including C#, R, Scala, and Python, which calls them [keyword arguments](https://docs.python.org/2/tutorial/controlflow.html#keyword-arguments).
...
```python
>>> n = 0b10101111  # one byte, as before
>>> n
175
>>> ~n
-176
...
'-0b10110000'
>>> bin(~n & mask)  # the bitwise complement :)
'0b1010000'
```
...
- Study the program and discuss the following aspects with your neighbour:
  - Where did `parser` parse from?! There must be some behind-the-scenes magic here?
  - Where did the usage string come from? i.e. `hello.py [-h] [-e num] name [name ...]`
  - What do the square brackets mean? Where did they come from?
...
The other arguments will be familiar from Assignment 1.

### Using  `with` statements

The following example and explanation are borrowed from the Python tutorial:
...
- Skim-read this document. Please feel encouraged to read it more carefully at your leisure.
